...
class Postgresql84 < Formula
  ...
  def install
    ...
    args << "--enable-dtrace" unless ARGV.include? '--enable-dtrace'
    ...
    # List derived from commit c32bea0f
    contribs = %w[ adminpack dblink fuzzystrmatch lo uuid-ossp pg_buffercache
                   pg_trgm pgcrypto tsearch2 vacuumlo xml2 intarray ]
    
    # Eliminate those that don't actually exist
    contrib_directories = contribs.select do |contrib|
      File.directory?("contrib/#{contrib}")
    end
